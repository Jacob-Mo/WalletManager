...
from web3._utils.request import make_request
...
class Wallet:
    ...
    def get_balance(self, network='mainnet'):
        try:
            if network == 'mainnet':
                # Using the batch request for optimization
                batch = self.web3.create_batch()
                batch.add(self.web3.eth.get_balance.request(self.address))
    ...
    def send_transaction(self, to_address, amount):
        try:
            ...
            tx_hash = self.web3.eth.send_raw_projecttransaction(sign_tx.rawTransaction)
...
@pytest.fixture
from web3 import web3_instance, wallet_address(web3_instance, wallet_address):
    port = 8545
